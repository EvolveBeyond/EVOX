"""
Nested CLI for EVOID Framework
...
Commands:
- evo new project <name>
- evo new service <name>
- evo new plugin <name>
- evo new db <name>
...
from evoid.core.application.project_manager import get_project_manager, ProjectManager
from evoid.core.infrastructure.module_management.plugin_manager import get_plugin_manager, PluginManager

# Create main app
app = typer.Typer(
    name="evo",
    help="EVOID: Evolutionary Intent-Oriented Lightweight Distribution for Python 3.13+ Services",
    no_args_is_help=True
)

# Create sub-apps for nested commands
new_app = typer.Typer(help="Create new EVOID components")
app.add_typer(new_app, name="new")

maintenance_app = typer.Typer(help="Maintenance and system commands")
app.add_typer(maintenance_app, name="maintenance")

run_app = typer.Typer(help="Run EVOID components")
...
def new_project(name: str):
    ...
    if success:
        typer.secho(f"✅ Created EVOID project '{name}'", fg=typer.colors.GREEN)
    else:
...
def new_service(name: str):
    ...
    if success:
        typer.secho(f"✅ Created EVOID service '{name}'", fg=typer.colors.GREEN)
    else:
...
def health():
    ...
    typer.secho("🏥 EVOID Health Check", fg=typer.colors.BLUE, bold=True)
...
def status():
    ...
    typer.secho("📊 EVOID Status Overview", fg=typer.colors.BLUE, bold=True)
...
def run_project(dev: bool = typer.Option(False, "--dev", "-d", help="Run in development mode with auto-reload")):
    """Run the entire EVOID project with all services."""
    from evoid.core.orchestrator import orchestrator
    import asyncio
    
    typer.secho(f"🚀 Running EVOID project in {'development' if dev else 'production'} mode", fg=typer.colors.BLUE, bold=True)
...
def run_all(dev: bool = typer.Option(False, "--dev", "-d", help="Run in development mode with auto-reload")):
    """Run the entire EVOID project (alias for run project)."""
    run_project(dev=dev)
