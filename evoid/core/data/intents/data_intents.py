...
class BaseIntentConfig:
    ...
    Enhanced for database-aware storage with storage_engine and consistency_level.
    """
    # Caching properties
    ...
    # Storage engine preference (sql, nosql, key-value, columnar, document)
    storage_engine: str = "auto"  # auto, sql, nosql, key_value, columnar, document
    
    # Consistency level for distributed systems
    consistency_level: str = "eventual"  # eventual, causal, strong, bounded
    
    # Database-specific properties
    database_properties: Dict[str, Any] = field(default_factory=dict)
    
    # Transaction requirements
    transaction_required: bool = False
    
    # Backup and durability settings
    backup_required: bool = False
    durability_level: str = "normal"  # normal, high, maximum
    
    # Custom properties for extension
    custom_properties: Dict[str, Any] = field(default_factory=dict)
    
    def __post_init__(self):
        """Set default TTL based on caching strategy and validate database properties"""
        ...
        # Validate storage engine values
        valid_engines = ["auto", "sql", "nosql", "key_value", "columnar", "document"]
        if self.storage_engine not in valid_engines:
            raise ValueError(f"Invalid storage_engine: {self.storage_engine}. Must be one of {valid_engines}")
        
        # Validate consistency level values
        valid_consistency = ["eventual", "causal", "strong", "bounded"]
        if self.consistency_level not in valid_consistency:
            raise ValueError(f"Invalid consistency_level: {self.consistency_level}. Must be one of {valid_consistency}")
        
        # Set database properties based on intent level
        if self.storage_engine == "auto":
            # Auto-select based on other properties
            if self.strong_consistency or self.transaction_required:
                self.storage_engine = "sql"
            elif self.cache_enabled and self.cache_aggressive:
                self.storage_engine = "key_value"
            else:
                self.storage_engine = "nosql"
